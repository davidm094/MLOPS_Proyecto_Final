...
│  │                         NAMESPACE: argocd                            │   │
│  │  ┌─────────────┐                                                     │   │
│  │  │  Argo CD    │◄────── Git Repository (GitHub)                      │   │
│  │  │  (GitOps)   │        - infra/argocd/applications/                 │   │
│  │  └──────┬──────┘        - apps/*/k8s/                                │   │
│  └─────────┼────────────────────────────────────────────────────────────┘   │
│            │ Sync                                                            │
│            ▼                                                                 │
...
│  │                                                                       │   │
│  │  ┌─────────────┐    ┌─────────────┐    ┌─────────────┐              │   │
│  │  │  Airflow    │    │   MLflow    │    │  SeaweedFS  │              │   │
│  │  │ (Scheduler) │───▶│  (Server)   │───▶│    (S3)     │              │   │
│  │  │ (Webserver) │    │             │    │             │              │   │
│  │  │ (Triggerer) │    └──────┬──────┘    └─────────────┘              │   │
│  │  └──────┬──────┘           │                   ▲                     │   │
│  │         │                  │                   │                     │   │
│  │         │ KubernetesExecutor                   │ Artifacts           │   │
│  │         ▼                  │                   │                     │   │
│  │  ┌─────────────┐           │           ┌──────┴──────┐              │   │
...
│                                                                              │
│  NODEPORT SERVICES:                                                         │
│  - Argo CD:   30443    - MLflow:    30500                                   │
│  - Airflow:   30080    - API:       30800                                   │
│  - Frontend:  30501                                                         │
└─────────────────────────────────────────────────────────────────────────────┘
```
...
```
┌──────────────┐     ┌──────────────┐     ┌──────────────┐     ┌──────────────┐
│   External   │     │   Airflow    │     │   SeaweedFS  │     │   MLflow     │
│     API      │────▶│   DAG        │────▶│     S3       │────▶│   Server     │
│ 10.43.100.103│     │              │     │              │     │              │
└──────────────┘     └──────┬───────┘     └──────────────┘     └──────────────┘
                           │                                          │
                    ...
                    ┌──────────────┐                           ┌──────────────┐
                    │   FastAPI    │◄──────────────────────────│   Model      │
                    │   /predict   │      Load Production      │  Artifacts   │
                    │   /explain   │      Model                │              │
                    └──────┬───────┘                           └──────────────┘
                           │
                           ▼
